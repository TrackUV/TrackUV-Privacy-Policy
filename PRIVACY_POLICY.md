...
Your location is used for a single purpose: to ask Apple's Weather service
for the current and forecast UV index at that location. The cached UV
reading is used to display the menu bar icon and the Notification Centre
widget without waiting on a fresh network request every time.
...
main app and the Notification Centre widget, on your device only, using
Apple's standard App Group mechanism. This never leaves your Mac.
